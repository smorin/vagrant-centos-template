...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  config.vm.define :demandcubecentostemplate1 do |demandcubecentostemplate1|
    # Create a private network, which allows host-only access to the machine
    # using a specific IP.
    demandcubecentostemplate1.vm.network :private_network, ip: "192.168.56.10"
    demandcubecentostemplate1.vm.hostname = "demandcubecentostemplate1.local"
    
    config.vm.provider :virtualbox do |vb|
      vb.name = "demandcubecentostemplate1"
      # vb.customize ["modifyvm", :id, "--memory", "1024"]
      # vb.customize ["modifyvm", :id, "--cpus", "2"]
    end
